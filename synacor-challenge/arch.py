...
class VM:
    ...
    def tic(self, address):
        ...
        if inst == 0:  # halt
            ...
            inst == 3
        ):  # pop a - remove the top element from the stack and write it to <a>; empty stack = error
            if self.stack:
                a = self.read(address + 1)
                val = self.stack.pop()
                self.write(a, val)
            ...
            inst == 4
        ):  # eq a b c - set <a> to 1 if <b> is equal to <c>; set to 0 otherwise
            a = self.read(address + 1)
            b = self.value(address + 2)
            ...
            inst == 5
        ):  # gt a b c - set <a> to 1 if <b> is greater than <c>; set to 0 otherwise
            a = self.read(address + 1)
            b = self.value(address + 2)
            c = self.value(address + 3)
            self.write(a, 1 if b > c else 0)
            ...
            inst == 19
        ):  # out a - write the character represented by the ascii code <a> to the terminal.
            a = self.value(address + 1)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        logging.info(
            f"Loading the eighth register with {value} and dissabling check..."
        )
        # vm.memory[521] = 21
        vm.memory[522] = HIGH
        # vm.memory[523] = 21
