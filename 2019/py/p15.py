...
class World:
  ...
  def shortest_path(self, start, new):
    ...
    came_from, cost_so_far = library.astar(wrap(start), wrap(new), heuristic, cost, wrapped_neighbors)
  ...
  def move_to(self, new):
    ...
    for next in path[1:]:
      ...
      if status == 0:
        ...
        if next != new:
          render(self, path)
          assert next == new, f"Hit wall too early. {self.loc} -> {next} -> {new}, {self.walls}, {path}"
      elif status == 1:
...
def render(world, path=[]):
  ...
  for y in range(ymin, ymax+1):
    for x in range(xmin, xmax+1):
      print("#" if x + 1j*y in world.walls else 
          ("o" if x+1j*y == world.oxygen else 
            ("x" if x+1j*y == 0 else 
              ("@" if x+1j*y == world.loc else
                ("." if (x,y) in set(path) else " ")))), end="")
    print()
