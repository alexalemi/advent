...
def make(inp: str) -> World:
  ...
  letter_locs = (
      loc for loc, c in board.items() if c in set(string.ascii_uppercase))
...
def gen_neighbors2(world):
  ...
  center = complex((bounds.xmax + bounds.xmin) / 2,
...
def answer2(inp):
  ...
  path = library.astar(
      ...
      heuristic=lambda x: x[0] + library.distance(
          library.unwrap(x[1]), world.end))
  return len(path) - 1
