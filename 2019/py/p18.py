...
class World:
  ...
  def __hash__(self):
    return hash((self.loc, self.moves, self.walls, tuple(self.keys.items()), tuple(self.doors.items())))
...
def make(inp: str) -> World:
  ...
  keys = {let: c for let, c in board.items() if c in set(string.ascii_lowercase)}
  doors = {c: let for let, c in board.items() if c in set(string.ascii_uppercase)}
...
def neighbors(world: World) -> Sequence[World]:
  for n in library.neighbors(world.loc):
    if n in world.walls:
      continue
    elif n in world.keys:
      # found a key
      key = world.keys[n]
      yield dataclasses.replace(
          ...
          doors={d:loc for d,loc in world.doors.items() if d.lower() != key})
    elif n in set(world.doors.values()):
      continue
    else:
      yield dataclasses.replace(world, loc=n, moves=world.moves + 1)
...
def answer1(inp):
  ...
  final, came_from, cost_so_far = library.astar(
    ...
    neighbors = neighbors,
	# heuristic = lambda world: len(world.keys))
	# heuristic = lambda world: min(library.distance(world.loc, loc) for loc in world.keys) if world.keys else 0)
	heuristic = lambda world: min(distance(world, loc) for loc in world.keys) if world.keys else 0)
