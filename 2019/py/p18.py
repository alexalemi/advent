...
class World:
    ...
    def __hash__(self):
        return hash(
            (self.locs, self.walls, tuple(sorted((n, x) for x, n in self.keys.items())))
        )
...
def neighbors(world: World) -> Sequence[World]:
    ...
    def valid(x):
        return (
            not (x in world.walls)
            and not (x in world.keys)
            and not (x in set(world.doors.values()))
        )

    frontier = [(world.moves, i, library.wrap(x)) for i, x in enumerate(world.locs)]
    seen = set()
    while frontier:
...
def replace_center(inp):
    ...
    return "\n".join(
        "".join(vals[x + y * 1j] for x in range(bounds.xmin, bounds.xmax + 1))
