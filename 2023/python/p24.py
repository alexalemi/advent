...
def collision(bounds: Bounds, ball0: Ball, ball1: Ball):
    px0, py0, _ = ball0.p
    vx0, vy0, _ = ball0.v
    px1, py1, _ = ball1.p
    vx1, vy1, _ = ball1.v
    if denom := vx1 * vy0 - vx0 * vy1:
        ...
        return (
            (t0 > 0)
            and (t1 > 0)
            and inside(bounds, move(ball0, t0))
            and inside(bounds, move(ball1, t1))
        )
    return False
...
def part1(bounds: Bounds, data: tuple[Ball, ...]):
    return sum(
        1
        for ball0, ball1 in itertools.combinations(data, 2)
        if collision(bounds, ball0, ball1)
    )
...
def solver(data):
    ...
    def project_xx_vv(xx, vv, ts, rho=rho):
        return (
            (rho * xx + (xs - ts[:, None] * (vv - vs)).sum(0)) / (rho + N),
            (rho * vv + (ts[:, None] * ((xs - xx) + ts[:, None] * vs)).sum(0))
            / (rho + (ts**2).sum()),
        )
