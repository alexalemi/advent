...
def raw_neighbors(z: State) -> list[State]:
    y, x = z.loc
    if z.direction == "v":
        return [State((y + 1, x), "v"), State((y, x + 1), ">"), State((y, x - 1), "<")]
    elif z.direction == "^":
        return [State((y - 1, x), "^"), State((y, x + 1), ">"), State((y, x - 1), "<")]
    elif z.direction == "<":
        return [State((y - 1, x), "^"), State((y + 1, x), "v"), State((y, x - 1), "<")]
    elif z.direction == ">":
        return [State((y - 1, x), "^"), State((y + 1, x), "v"), State((y, x + 1), ">")]
...
def part1(data: Data) -> int:
    ...
    def neighbors(state: State) -> list[State]:
        if state.loc in data.slopes:
            return [
                State(
                    raw_slide(state.loc, data.slopes[state.loc]), data.slopes[state.loc]
                )
            ]
        return [
            neigh
            for neigh in raw_neighbors(state)
            if inside(neigh)
            and neigh
            and (neigh.loc not in data.forests)
            and not incompatible(neigh)
        ]
...
def part2(data: Data) -> int:
    ...
    def neighbors(state: State) -> list[State]:
        return [
            neigh
            for neigh in raw_neighbors(state)
            if inside(neigh) and (neigh not in data.forests)
        ]
...
def condense(data: Data) -> int:
    ...
    while paths:
        ...
        neighs = [
            neigh
            for neigh in neighbors(loc)
            if (neigh != prev) and (neigh not in data.forests)
        ]
        if len(neighs) > 1:
            # at a branch point
