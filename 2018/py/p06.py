...
dists = np.abs((np.mgrid[0:width, 0:height] +
                np.array([left, top])[:, None, None]) -
               coords[:, :, None, None]).sum(1)
...
edgeguys = set(fillboard[:, 0]).union(set(fillboard[0, :])).union(
    set(fillboard[:, -1])).union(set(fillboard[-1, :]))
