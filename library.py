...
def render(bounds: Bounds, world: Mapping[Location, Value], symbol: Callable[[Value], chr], make=lambda x,y: x+1j*y):
...
def astar(
    start: Location, 
    goal: Location, 
    heuristic: Callable[[Location, Location], Score],
    cost: Callable[[Location, Location], Score],
    neighbors: Callable[[Location], List[Location]])-> Tuple[Dict[Location, Location], Dict[Location, Score]]:
  frontier = []
