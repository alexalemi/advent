...
def generate_checks(inp):
  ...
  for line in parts.splitlines():
    ...
    checks[leader] = [
        tuple(map(int, interval.split('-'))) for interval in ranges
    ]
  return checks
...
def answer1(inp):
  ...
  nearbys = inp.split('\n\n')[-1].splitlines()[1:]
  results = filter(lambda x: not test(x),
                   (int(x) for line in nearbys for x in line.split(',')))
  return sum(results)


tests2 = [("""departure class: 0-1 or 4-19
row: 0-5 or 8-19
seat: 0-13 or 16-19
...
def answer2(inp):
  ...
  potentials = [
      set(f
          for f, test in check_tests.items()
          if all(map(test, field)))
      for field in fields
  ]
