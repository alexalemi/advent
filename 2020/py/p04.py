...
hgt = p.assertion(
    p.chain(
        p.ignore(p.exact("hgt:")),
        p.integer,
        p.alternatives([p.exact("cm"), p.exact("in")]),
    ),
    valid_height,
)
...
pid = head(p.chain(p.ignore(p.exact("pid:")), p.map(p.repeat(p.digit, 9), p.join)))

cid = head(p.chain(p.ignore(p.exact("cid:")), p.map(p.plus(p.non_whitespace), p.join)))
